...
function logify(req, res, logger) {
    ...
    res.end = function () {
        ...
        logger((req.socket && req.socket.remoteAddress) + " - - [" + (new Date()).toUTCString() + "]" + " \"" + req.method + " " + req.url + " HTTP/" + req.httpVersionMajor + "." + req.httpVersionMinor + "\" " + res.statusCode + " - \"" + (req.headers['referer'] || "") + "\" \"" + (req.headers["user-agent"] ? req.headers["user-agent"].split(' ')[0] : '') + "\"");
                    ...
                    if (match && match[0].length > 0) {
                        ...
                        switch(route.method)
                        {
                            ...
                            case 'POST':
                                ...
                                req.on('end', function () {
                                    handler(req, res, querystring.parse(body));
                                });
                                break;
                            default:
...
exports.staticDirHandler = function (root, prefix) {
    ...
    return function (req, res) {
        ...
        if (filename == root) {
            filename = path.join(root, 'states.html');
        }
...
var mime = exports.mime = {
    ...
    getMime:function getMime(path) {
        ...
        if (index < 0) { return DEFAULT_MIME; }
